...
$chat_user_name = isset($_GET['chat_user_name']) ? htmlspecialchars($_GET['chat_user_name'])  : "";
...
$version_num = isset($_GET['version_num']) ? (int)htmlspecialchars($_GET['version_num'])  : 101;
...
if($mac!=""){
    ...
    if($select_device){
        if($user_sum<$select_device[0]["max_user_sum"]){
            ...
            // $add_user=$add_user_status[0]["add_user_status"];
            if($add_user_status[0]["add_user_status"]=="正在上号"){
                $add_user=1;
            };
        };
